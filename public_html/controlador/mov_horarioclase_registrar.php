...
if(isset($_POST) && !empty($_POST)){
	...
	if ($lb_result == true) {
		...
		// Definir estructura
		$array_campo	= array('N_COD_HORARIO', 'N_CLASE', 'N_COD_INSTRUCTOR', 'D_FEC_PROG', 'D_HORA_PROG', 
								'N_HORAS', 'V_OBSERVACION', 'V_FLAG_ESTADO', 'V_AUD_USR_REG', 'D_AUD_FEC_REG');
		$array_valor	= array($li_cod_horario, $li_clase, $li_cod_instructor, $li_fec_prog, $li_hora_prog,
								$li_horas, $ls_observacion, $ls_flag_estado, $_SESSION['usr_conectado'], $ldt_fecha_insercion);
		
		// Invocar inserción
		$lb_result = $crud->fila_registrar(DEF_TABLA_HORARIOPROG, $array_campo, $array_valor, '0');
	...
	if($lb_result == true) {
		...
		header("Location: ../vista/mov_horarioclase_nuevo.php?id_codigo_padre=".$li_cod_horario."&id_respuesta='".$ls_mensaje."'"); 
	}
