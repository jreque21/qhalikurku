...
if(isset($_GET['id_codigo'])){
	...
	if ( $li_contador > 0 ) {
		$ls_mensaje	= 'Horario ya se encuentra aperturado.';
	...
	$array_valor	= array('2', $_SESSION['usr_conectado'], $ldt_fecha_actualizacion);
	...
	// Redireccionar
	if($lb_result == true) {
		$ls_mensaje	= 'OK-Horario se aperturó de forma satisfactoria.';
		header("Location: ../vista/mov_horario_lista.php?id_msgRpta=".$ls_mensaje);
	}
	else {
		$ls_mensaje	= 'Inconvenientes al aperturar horario.';
		header("Location: ../vista/mov_horario_lista.php?id_msgRpta=".$ls_mensaje);
	}
