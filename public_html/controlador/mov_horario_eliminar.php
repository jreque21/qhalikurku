...
if ( $li_contador > 0 ) {
	$ls_mensaje	= 'Horario se encuentra referenciado en Programación.';
}
if ($li_contador == 0) {
	$li_contador = $crud->fila_contar(DEF_TABLA_MATRICULA, $array_campo_pk, $array_valor_pk);
	if ( $li_contador > 0 ){
		$ls_mensaje	= 'Horario se encuentra referenciado en registros de Matricula.';
	}
}
...
if ( $li_contador > 0 ) {
	// Redireccionar
	if($lb_form == true) {
		header("Location: ../vista/mov_horario_editar.php?id_codigo=".$li_codigo."&id_msgRpta=".$ls_mensaje);
	} else {
...
// Invocar CRUD
$lb_result = $crud->fila_eliminar(DEF_TABLA_HORARIO, $array_campo_pk, $array_valor_pk);
...
if($lb_result == true) {
	...
	header("Location: ../vista/mov_horario_editar.php?id_codigo=".$li_codigo);
}
