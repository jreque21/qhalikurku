...
function f_listado($as_titulo, $as_icono, $as_msgRpta){
	...
	// Enlaces
	$url_nuevo		= "mae_tipoterapia_nuevo.php";
	$url_editar		= "mae_tipoterapia_editar.php";
	$url_lista		= "mae_tipoterapia_lista.php";
	$url_eliminar	= "../controlador/mae_tipoterapia_eliminar.php";
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_TIPOTERAPIA, $array_campo_pk, $array_valor_pk, 'V_DES_CORTA', 'A', 0, 100);
									...
									<table id="lista" class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											while ($row = mysqli_fetch_assoc($array)) {
												$url_editar_fila   = $url_editar."?id_codigo=".($row["N_COD_TIPOTERAPIA"]);
												$url_eliminar_fila = $url_eliminar."?id_codigo=".($row["N_COD_TIPOTERAPIA"]);
												?>
												<tr>
													<?php

													echo "<td>";
													echo f_r_url($url_editar_fila, $row["N_COD_TIPOTERAPIA"],$row["V_DES_LARGA"]);
													echo "</td>";
													...
													echo "<td align='center'>";
													?>
													<a href="#deleteModal<?php echo $row["N_COD_TIPOTERAPIA"]; ?>" data-toggle="modal" ><i class="fa fa-trash-o"></i></a>
													<div id="deleteModal<?php echo $row["N_COD_TIPOTERAPIA"]; ?>" class="modal fade">
...
function f_formulario($as_titulo, $as_icono, $as_msgRpta, $array = "") {
	...
	// Enlaces
	$url_lista		= "mae_tipoterapia_lista.php";
	$url_registrar	= "../controlador/mae_tipoterapia_registrar.php";
	$url_actualizar	= "../controlador/mae_tipoterapia_actualizar.php";
	$url_eliminar	= "../controlador/mae_tipoterapia_eliminar.php";
							...
							<form  id="form_mtto" role="form" method="post" action="" enctype="multipart/form-data" autocomplete="off">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_codigo" name="id_codigo" maxlength="5" disabled
										title = "Generado por el sistema" placeholder="Codigo autogenerado" value="<?php echo $lb_edit?$array["N_COD_ESPECIALIDAD"]:""; ?>">
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_larga" name="id_des_larga" maxlength="150" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,150}" autofocus
										title = "Letras y Números. Tamaño máximo: 150" placeholder="(*) Ejemplo : Transferencia" value="<?php echo $lb_edit?$array["V_DES_LARGA"]:""; ?>">
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_corta" name="id_des_corta" maxlength="100" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,100}"
										title = "Letras y Números. Tamaño máximo: 100" placeholder="(*) Ejemplo : Transf." value="<?php echo $lb_edit?$array["V_DES_CORTA"]:""; ?>">
								</div>
								
								<div class="form-group">
									<label for="id_duracion_min">Duración (Min.)</label>
									<input type="number" class="form-control" id="id_duracion_min" name="id_duracion_min" maxlength="9" min="0" required
									title = "Campo numérico" placeholder="(*) Ejemplo : 60" value="<?php echo $lb_edit?$array["N_DURACION_MIN"]:""; ?>">
								</div>
								
								<div class="form-group">
									<label for="id_precio">Precio</label>
									<input type="number" class="form-control" id="id_precio" name="id_precio" maxlength="9" min="0" required
									title = "Campo numérico" placeholder="(*) Ejemplo : 12.75" value="<?php echo $lb_edit?$array["N_PRECIO"]:""; ?>">
								</div>
								...
								<?php
								if ($lb_edit) {
									echo "<input type=hidden name=id_codigo value=\"".$array["N_COD_TIPOTERAPIA"]."\">";
									?>
