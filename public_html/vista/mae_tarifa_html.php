...
function f_listado($as_titulo, $as_icono, $as_msgRpta){
	...
	// Enlaces
	$url_nuevo		= "mae_tarifa_nuevo.php";
	$url_editar		= "mae_tarifa_editar.php";
	$url_lista		= "mae_tarifa_lista.php";
	$url_eliminar	= "../controlador/mae_tarifa_eliminar.php";
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_TARIFA, $array_campo_pk, $array_valor_pk, 'V_DES_CORTA', 'A', 0, 100);
									...
									<table id="lista" class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											while ($row = mysqli_fetch_assoc($array)) {
												$url_editar_fila   = $url_editar."?id_codigo=".($row["N_COD_TARIFA"]);
												$url_eliminar_fila = $url_eliminar."?id_codigo=".($row["N_COD_TARIFA"]);
												?>
												<tr>
													<?php
													echo "<td>";
													echo f_r_url($url_editar_fila, $row["N_COD_TARIFA"],$row["V_DES_LARGA"]);
													echo "</td>";
													...
													$ls_moneda		= $crud->fila_recuperar_campo('MAE_MONEDA', $array_campo_pk, $array_valor_pk, 'V_DES_CORTA');
													...
													echo "<td align='center'>";
													?>
													<a href="#deleteModal<?php echo $row["N_COD_TARIFA"]; ?>" data-toggle="modal" ><i class="fa fa-trash-o"></i></a>
													<div id="deleteModal<?php echo $row["N_COD_TARIFA"]; ?>" class="modal fade">
...
function f_formulario($as_titulo, $as_icono, $as_msgRpta, $array = "") {
	...
	// Enlaces
	$url_lista		= "mae_tarifa_lista.php";
	$url_registrar	= "../controlador/mae_tarifa_registrar.php";
	$url_actualizar	= "../controlador/mae_tarifa_actualizar.php";
	$url_eliminar	= "../controlador/mae_tarifa_eliminar.php";
							...
							<form  id="form_mtto" role="form" method="post" action="" enctype="multipart/form-data" autocomplete="off">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_codigo" name="id_codigo" maxlength="5" disabled
										title = "Generado por el sistema" placeholder="Codigo autogenerado" value="<?php echo $lb_edit?$array["N_COD_TARIFA"]:""; ?>">
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_larga" name="id_des_larga" maxlength="150" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,150}" autofocus
										title = "Letras y Números. Tamaño máximo: 150" placeholder="(*) Ejemplo : Tarifa General" value="<?php echo $lb_edit?$array["V_DES_LARGA"]:""; ?>">	 
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_corta" name="id_des_corta" maxlength="100" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,100}"
										title = "Letras y Números. Tamaño máximo: 100" placeholder="(*) Ejemplo : General" value="<?php echo $lb_edit?$array["V_DES_CORTA"]:""; ?>">
								</div>
								...
								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
											...
											<input type="number" class="form-control" id="id_monto" name="id_monto" maxlength="9" step="0.01" min="0" required 
											title = "Campo numérico" placeholder="(*) Ejemplo : 12.75" value="<?php echo $lb_edit?$array["N_MONTO"]:""; ?>">
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											<label for="id_descuento">Descuento</label>
											<input type="number" class="form-control" id="id_descuento" name="id_descuento" maxlength="9" step="0.01" min="0"  
											title = "Campo numérico" placeholder="Ejemplo : 12.75" value="<?php echo $lb_edit?$array["N_DESCUENTO"]:""; ?>">
										</div>
									</div>
								</div>	
								...
								<?php
								if ($lb_edit) {
									echo "<input type=hidden name=id_codigo value=\"".$array["N_COD_TARIFA"]."\">";
									?>
