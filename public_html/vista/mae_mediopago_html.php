...
function f_listado($as_titulo, $as_icono, $as_msgRpta){
	...
	// Enlaces
	$url_nuevo		= "mae_mediopago_nuevo.php";
	$url_editar		= "mae_mediopago_editar.php";
	$url_lista		= "mae_mediopago_lista.php";
	$url_eliminar	= "../controlador/mae_mediopago_eliminar.php";
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_MEDIOPAGO, $array_campo_pk, $array_valor_pk, 'V_DES_CORTA', 'A', 0, 100);
									...
									<table id="lista" class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											while ($row = mysqli_fetch_assoc($array)) {
												$url_editar_fila   = $url_editar."?id_codigo=".($row["N_COD_MEDIOPAGO"]);
												$url_eliminar_fila = $url_eliminar."?id_codigo=".($row["N_COD_MEDIOPAGO"]);
												?>
												<tr>
													<?php

													echo "<td>";
													echo f_r_url($url_editar_fila, $row["N_COD_MEDIOPAGO"],$row["V_DES_LARGA"]);
													echo "</td>";
													...
													echo "<td align='center'>";
													?>
													<a href="#deleteModal<?php echo $row["N_COD_MEDIOPAGO"]; ?>" data-toggle="modal" ><i class="fa fa-trash-o"></i></a>
													<div id="deleteModal<?php echo $row["N_COD_MEDIOPAGO"]; ?>" class="modal fade">
...
function f_formulario($as_titulo, $as_icono, $as_msgRpta, $array = "") {
	...
	// Enlaces
	$url_lista		= "mae_mediopago_lista.php";
	$url_registrar	= "../controlador/mae_mediopago_registrar.php";
	$url_actualizar	= "../controlador/mae_mediopago_actualizar.php";
	$url_eliminar	= "../controlador/mae_mediopago_eliminar.php";
					...
					<div class="panel panel-primary">
						...
						<div class="panel-body">
							...
							<form  id="form_mtto" role="form" method="post" action="" enctype="multipart/form-data" autocomplete="off">
								...
								<?php
								if ($lb_edit) {
									echo "<input type=hidden name=id_codigo value=\"".$array["N_COD_MEDIOPAGO"]."\">";
									?>
