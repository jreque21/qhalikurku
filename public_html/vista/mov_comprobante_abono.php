...
// Almacena ID (comprobante al que se le va a abonar)
$id = isset($_GET["id_codigo"]) ? $_GET["id_codigo"] : '';
...
if (empty($id)) {
	header('Location: mov_comprobante_lista.php');
	exit;
}
...
f_formulario_abono($array_opcion['V_ETIQUETA'], $array_opcion['V_ICONO'], $ls_msgRpta, $id);
f_admin_pie();
f_admin_script('', '');
