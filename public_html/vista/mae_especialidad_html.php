...
function f_listado($as_titulo, $as_icono, $as_msgRpta){
	...
	// Enlaces
	$url_nuevo		= "mae_especialidad_nuevo.php";
	$url_editar		= "mae_especialidad_editar.php";
	$url_lista		= "mae_especialidad_lista.php";
	$url_eliminar	= "../controlador/mae_especialidad_eliminar.php";
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_ESPECIALIDAD, $array_campo_pk, $array_valor_pk, 'V_DES_CORTA', 'A', 0, 100);
									...
									<table id="lista" class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											while ($row = mysqli_fetch_assoc($array)) {
												$url_editar_fila   = $url_editar."?id_codigo=".($row["N_COD_ESPECIALIDAD"]);
												$url_eliminar_fila = $url_eliminar."?id_codigo=".($row["N_COD_ESPECIALIDAD"]);
												?>
												<tr>
													<?php

													echo "<td>";
													echo f_r_url($url_editar_fila, $row["N_COD_ESPECIALIDAD"],$row["V_DES_LARGA"]);
													echo "</td>";
													...
													echo "<td align='center'>";
													?>
													<a href="#deleteModal<?php echo $row["N_COD_ESPECIALIDAD"]; ?>" data-toggle="modal" ><i class="fa fa-trash-o"></i></a>
													<div id="deleteModal<?php echo $row["N_COD_ESPECIALIDAD"]; ?>" class="modal fade">
...
function f_formulario($as_titulo, $as_icono, $as_msgRpta, $array = "") {
	...
	// Enlaces
	$url_lista		= "mae_especialidad_lista.php";
	$url_registrar	= "../controlador/mae_especialidad_registrar.php";
	$url_actualizar	= "../controlador/mae_especialidad_actualizar.php";
	$url_eliminar	= "../controlador/mae_especialidad_eliminar.php";
					...
					<div class="panel panel-primary">
						...
						<div class="panel-body">
							...
							<form  id="form_mtto" role="form" method="post" action="" enctype="multipart/form-data" autocomplete="off">
								...
								<?php
								if ($lb_edit) {
									echo "<input type=hidden name=id_codigo value=\"".$array["N_COD_ESPECIALIDAD"]."\">";
									?>
