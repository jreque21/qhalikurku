...
function f_listado($as_titulo, $as_icono, $as_msgRpta){
	...
	// Enlaces
	$url_nuevo		= "mae_tipodoc_nuevo.php";
	$url_editar		= "mae_tipodoc_editar.php";
	$url_lista		= "mae_tipodoc_lista.php";
	$url_eliminar	= "../controlador/mae_tipodoc_eliminar.php";
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_TIPO_DOC, $array_campo_pk, $array_valor_pk, 'V_DES_CORTA', 'A', 0, 100);
									...
									<table id="lista" class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											while ($row = mysqli_fetch_assoc($array)) {
												$url_editar_fila   = $url_editar."?id_codigo=".($row["N_COD_TIPODOC"]);
												$url_eliminar_fila = $url_eliminar."?id_codigo=".($row["N_COD_TIPODOC"]);
												?>
												<tr>
													<?php
													echo "<td>";
													echo f_r_url($url_editar_fila, $row["N_COD_TIPODOC"],$row["V_DES_LARGA"]);
													echo "</td>";
													...
													echo "<td align='center'>";
													?>
													<a href="#deleteModal<?php echo $row["N_COD_TIPODOC"]; ?>" data-toggle="modal" ><i class="fa fa-trash-o"></i></a>
													<div id="deleteModal<?php echo $row["N_COD_TIPODOC"]; ?>" class="modal fade">
...
function f_formulario($as_titulo, $as_icono, $as_msgRpta, $array = "") {
	...
	// Enlaces
	$url_lista		= "mae_tipodoc_lista.php";
	$url_registrar	= "../controlador/mae_tipodoc_registrar.php";
	$url_actualizar	= "../controlador/mae_tipodoc_actualizar.php";
	$url_eliminar	= "../controlador/mae_tipodoc_eliminar.php";
							...
							<form  id="form_mtto" role="form" method="post" action="" enctype="multipart/form-data" autocomplete="off">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_codigo" name="id_codigo" maxlength="5" disabled
										title = "Generado por el sistema" placeholder="Codigo autogenerado" value="<?php echo $lb_edit?$array["N_COD_TIPODOC"]:""; ?>">
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_larga" name="id_des_larga" maxlength="150" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,150}" autofocus
										title = "Letras y Números. Tamaño máximo: 150" placeholder="(*) Ejemplo : Documento Nacional de Identidad" value="<?php echo $lb_edit?$array["V_DES_LARGA"]:""; ?>"> 
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_corta" name="id_des_corta" maxlength="100" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,100}"
										title = "Letras y Números. Tamaño máximo: 100" placeholder="(*) Ejemplo : DNI" value="<?php echo $lb_edit?$array["V_DES_CORTA"]:""; ?>">
								</div>
								...
								<div class="form-group">
									...
									<?php
									if($lb_edit) {
										$li_longitud = $array["N_LONGITUD"];
									}else{	
										$li_longitud = 0;
									}
									?>
									<select class="form-control" id="id_longitud" name="id_longitud" required>
									<option value='' selected disabled hidden>Seleccione opción</option>
									<option value="8"<?php if("8" == $li_longitud) echo "selected";?>>8</option>
									<option value="9"<?php if("9" == $li_longitud) echo "selected";?>>9</option>
									<option value="10"<?php if("10" == $li_longitud) echo "selected";?>>10</option>
									<option value="11"<?php if("11" == $li_longitud) echo "selected";?>>11</option>
									<option value="12"<?php if("12" == $li_longitud) echo "selected";?>>12</option>
									<option value="13"<?php if("13" == $li_longitud) echo "selected";?>>13</option>
									<option value="14"<?php if("14" == $li_longitud) echo "selected";?>>14</option>
									<option value="15"<?php if("15" == $li_longitud) echo "selected";?>>15</option>
									</select>			
								</div>
								...
								<?php
								if ($lb_edit) {
									echo "<input type=hidden name=id_codigo value=\"".$array["N_COD_TIPODOC"]."\">";
									?>
